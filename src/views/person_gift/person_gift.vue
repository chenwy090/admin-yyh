        ...
        <Form inline>
          ...
          <span v-if="drop">
            <FormItem label="优惠券活动名称：" :label-width="120">
              <Input style="width:100%" v-model="searchData.name" placeholder="请输入优惠券活动名称" clearable />
            </FormItem>
          </span>
        ...
        <div>
          ...
          <Table border width="100%" :columns="columns8" :data="personGiftList" :loading="TableLoading">
            ...
            <template slot-scope="{ row }" slot="action">
              <Button type="primary" size="small" style="margin-right: 5px" @click="editStaff(row)">编辑</Button>
              <Button type="success" style="margin-right: 5px" size="small" @click="checkDetails(row)">查看详情</Button>
    ...
    >
      <Form ref="formValidate" :model="formValidate" :label-width="120">
        <FormItem label="奖品优惠券活动：" required>
          <Button @click="getCampaginListFn">
            {{ campaginName || "未选择" }}
          </Button>
        </FormItem>
        <Row>
          <Timeline pending>
            <TimelineItem v-for="(item, index) in formValidate.cashCouponDrawSetList" :key="index" style="padding:0;">
              <Row>
                <Col span="2">
                ...
                <Col span="22">
                  <Card>
                    ...
                    <FormItem
                      ...
                      :label-width="110"
                    >
                      <InputNumber placeholder="请选择时间" style="width: 120px" disabled v-model="item.percent" />
                    </FormItem>
                    <Button
                      type="error"
                      size="small"
...
export default {
  ...
  methods: {
    ...
    // 选择优惠活动对话框
    getCampaginListFn() {
      getCampagin_6List({ requestFrom: "newRedEnvelopes" }).then(res => {
        if (res.code == 200) {
          // console.log(res);
