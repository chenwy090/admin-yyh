      ...
      <Col>
        ...
        <Card>
          ...
          <Row class="operation">
            ...
            发放<span style="color:red">{{queryCount.stockCount}}</span>张
            卖出<span style="color:red">{{queryCount.receiveCount}}</span>张
        ...
        tableColumns: [
          ...
          },
          {
            ...
            render: (h, params) => {
              ...
              const color = row.status == "1"?"blue":
                      row.status==2?"#2db7f5":"#ed4014";
              var barcodeTemp = row.barcode;
              var prefix = barcodeTemp.substr(0,2);
              var midfix = "*********";
              var subfix = barcodeTemp.substr(-2,2);
              const text = row.status == "1"? prefix+midfix+subfix :row.barcode;
          ...
          },
          {
            ...
            render: (h, params) => {
              ...
              const color = row.status == "1"?"blue":
                      row.status==2?"#2db7f5":"#ed4014";
              const text = row.status == "1"?"未使用":row.status == "2"?"已使用":"已过期" ;
    ...
    methods: {
      ...
      updateTableList() {
        ...
        const reqParams = {
          ...
          receivedFrom:this.searchItem.receivedFrom,
