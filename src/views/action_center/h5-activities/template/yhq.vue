<template>
	<div>
		...
		<!--优惠券的详细信息-->
		<Modal v-model="yhqDetail" :closable="true" :mask-closable="false" :styles="{top: '20px'}" width="500"  :title="yhqTitle" @on-cancel="closeDialog">
				...
				<Form label-position="right"
		          ...
		          :label-width="100">
					<FormItem label="选择优惠券：" prop="templateId">
						<Input style="width:200px;margin-right:5px;display:inline-table;vertical-align:middle" v-model="formData.templateId" disabled>
		...
		methods:{
		    ...
		    handleSubmitSave(name){
		    	this.$refs[name].validate( async valid => {
		    		if(valid){
		    			const {formData:{id,templateId,surplusCount,beginTime,endTime,isYg,isYgValue,img,mainTitle,subTitle},searchData:{type},moduleId,browsingId} = this;
		    			let params = {
