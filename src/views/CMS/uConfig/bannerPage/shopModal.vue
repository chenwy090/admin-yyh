<template>
  <Modal width="800" v-model="viewDialogVisible" title="内容选择" :closable="false" :mask-closable="false" footer-hide>
    <div>
      <div>
        <Card :bordered="false" style="margin-bottom:2px">
          <Form ref="modalForm" label-position="right" :label-width="100" :model="modalForm" inline>
            ...
            <FormItem :label-width="0">
              <span>所在地区：</span>
              <Select v-model="modalForm.provinceId" style="width:150px" clearable @on-change="getcitylist">
                <Option v-for="(item, index) in provincelist" :key="index" :value="item.provinceCode">{{
                  item.provinceName
                }}</Option>
              </Select>
              <Select v-model="modalForm.cityId" style="width:150px" clearable @on-change="getarealist">
...
export default {
  ...
  methods: {
    ...
    //根据省份code获取城市信息数据
    getcitylist() {
      getRequest('/system/area/city/' + this.modalForm.provinceId).then(res => {
        if (res.code == 200) {
    ...
    //根据城市code获取区县信息数据
    getarealist() {
      getRequest('/system/area/district/' + this.modalForm.cityId).then(res => {
        if (res.code == 200) {
    ...
    loadTableData(page) {
      ...
      postRequest('/merchant/merchantInfo/list?pageNum=' + this.modalForm.current + '&pageSize=' + 10, params).then(
        res => {
