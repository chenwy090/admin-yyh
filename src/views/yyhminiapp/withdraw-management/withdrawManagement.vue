	    ...
	    <div style="margin-top: 15px;" >
	        <Form :model="add_info" ref="add_info" :label-width="180" >
	        	<FormItem label="提现规则说明：" required>
	            	<Input type="text" v-model="add_info.cashDesc" style="width:300px"/>
	          	</FormItem>

	        	<FormItem label="提现最低金额："  required>
	        		<InputNumber v-model="add_info.cashMin" style="width:300px" :min="0" ></InputNumber>&nbsp;&nbsp;元
	          	</FormItem>

	        	<FormItem label="提现阶梯：" required>
	        		<InputNumber v-model="add_info.cashStep" style="width:300px" :min="0" disabled></InputNumber>&nbsp;&nbsp;元
	          	</FormItem>

	        	<FormItem label="每人每天最多申请提现次数："  required>
	        		<InputNumber v-model="add_info.dailyMaxCount" style="width:300px" :min="0" ></InputNumber>
	            </FormItem>
	            
	            <FormItem style="{'margin-top':'54px'}">
             		<Button type="primary" size="large" @click="saveRule" :loading="add_loading" :style="{width:'230px'}" >保存</Button >
...
export default {
  ...
  methods: {
    ...
    getWithdrawCashRuleList() {
      postRequest("/rule/withdrawCashRule/selectWithdrawCashRuleList").then(res => {
