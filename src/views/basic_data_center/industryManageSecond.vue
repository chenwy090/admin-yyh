...
<template>
    <div class="search">
        ...
        <span v-if="edit_info.firstPage && !edit_info.industryIsSecond">
          <FormItem label="一级行业" >
            <Input v-model="edit_info.name" placeholder="请输入" clearable/>
          </FormItem>
         </span>
          ...
          <FormItem label="二级行业" >
            <Input v-model="edit_info.name" placeholder="请输入" clearable/>
                ...
                <FormItem label="排序" prop="orderBy">
                    <Input v-model="edit_info.orderBy" placeholder="请输入" clearable/>
                </FormItem>
                <FormItem label="备注">
                    <Input
    ...
    export default {
        ...
        methods: {
            ...
            updateTableList() {
                ...
                postRequest(
                    "/merchant/industrySecond/list?pageNum=" +
                    this.pageNum +
                    "&pageSize=" +
                    this.limit,
                    reqParams
            ...
            updateStatusInfo(item) {
                const self = this;
                const text=item.status==1?"您确定要将记录修改为隐藏状态吗？"
                    :"确定要将记录修改为显示状态吗？";
                ...
                this.$Modal.confirm({
                    ...
                    onOk: function() {
                        self.$Loading.start();
                        postRequest("/merchant/industrySecond/statusEdit?id=" + item.id + "&status="+consStatus).then(
                            res => {
                                self.loading = false;
