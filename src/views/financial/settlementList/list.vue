        ...
        <template slot-scope="{ row }" slot="operate">
          ...
          <div v-if="row.generateStatus === 1">
            <Button
              type="error"
              size="small"
              :disabled="!(row.billStatus == 1 || row.billStatus == 2)"
              @click="settleBillDelete(row)"
              >删除</Button
            >
          </div>
          <div v-if="row.generateStatus !== 1">
            <ButtonGroup>
              ...
              <Button
                ...
                size="small"
                :disabled="!(row.billStatus == 1 || row.billStatus == 2)"
                @click="editLabelDisplayFn(row)"
                >申请付款</Button
              >
              <!-- 只有状态为"被驳回"状态时，才会显示"重新生成"按钮。 -->
              <Button type="primary" size="small" :disabled="!(row.billStatus == 2)" @click="settleBillRegen(row)"
                >重新生成</Button
              >
            ...
            <Col span="12">
              <FormItem prop="settleTimeStart">
                <DatePicker
                  ...
                  "
                  format="yyyy-MM-dd"
                  v-model="modalAddData.settleTimeStart"
                  placeholder="请选择结算周期"
                  style="width: 150px"
                >
                </DatePicker>
            ...
            <Col span="12">
              <FormItem prop="settleTimeEnd">
                <DatePicker
                  ...
                  "
                  format="yyyy-MM-dd"
                  v-model="modalAddData.settleTimeEnd"
                  placeholder="请选择结算周期"
                  style="width: 150px"
                >
                </DatePicker>
