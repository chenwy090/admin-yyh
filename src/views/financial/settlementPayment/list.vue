      ...
      <Form ref="searchForm" :model="searchForm" inline :label-width="90" class="search-form">
        ...
        <FormItem label="结算付款时间" prop="payTime">
          <DatePicker
            transfer
            type="daterange"
            placeholder="请选择结算付款时间"
            @on-change="searchForm.payTime = $event"
      ...
      <Table :loading="TableLoading" border :columns="tableColumns" :data="table_list" sortable="custom" ref="table">
        <template slot-scope="{ row }" slot="operate">
          <ButtonGroup>
            ...
            <Button :disabled="!(row.billStatus == 3)" type="success" size="small" @click="settleBillCheckPayFn(row)"
              >确认付款</Button
            >
            <!-- 只有状态为"待付款"时，才显示"驳回"按钮。 -->
            <Button :disabled="!(row.billStatus == 3)" type="primary" size="small" @click="modalDisplayShowFu(row)"
              >驳回</Button
            >
